...
# Function to strip space and '/' characters and convert resulting string to lower case
def clean_data(x):
    if isinstance(x, str):
        x = x.replace(" ", "")
        x = x.replace("/", "")
...
# Relevant product metadata features
features = ['product_type_name', 'graphical_appearance_name', 'colour_group_name', 'index_name', 'garment_group_name']
...
for feature in features:
    articles_df2[feature] = articles_df2[feature].apply(clean_data)  
articles_df2['product_metadata_string'] = articles_df2['product_type_name'] + ' ' + articles_df2['graphical_appearance_name'] + ' ' + articles_df2['colour_group_name'] + ' ' + articles_df2['index_name'] + ' ' + articles_df2['garment_group_name']
...
articles_df2['product_metadata_string'] = articles_df2['product_metadata_string'].fillna('')

# Record start time before constructing TF-IDF matrix
# Construct the required TF-IDF matrix by fitting and transforming the data
start = time.time()
tfidf_matrix = tfidf.fit_transform(articles_df2['product_metadata_string'])
...
top_selling_articles_by_number_array = transactions_train_df.groupby('article_id').count()['t_dat'].sort_values(ascending=False).values

if IS_TEST.lower() == 'true':
    iterations = 0
    MAX_ITERATIONS_FOR_TEST = 10
    
for row in customers_df.head(10).itertuples():
    if IS_TEST.lower() == 'true':
        iterations += 1
    ...
    if customer_id in transactions_train_df.values:
        ...
        list_customer_id.append(customer_id)
        recommended_articles_string = ''
        for i in top_selling_articles_by_number_array[:NUM_RECOMMENDATIONS_PER_CUSTOMER]:
            recommended_articles_string += str(i.item())
            recommended_articles_string += str(' ')
...
submission_content_based_product_metadata_string_df = pd.DataFrame(list(zip(list_customer_id, list_prediction)), columns =['customer_id', 'prediction'])
submission_content_based_product_metadata_string_df.to_csv('submission_content_based_product_metadata_string.csv', encoding='utf-8', index=False)
submission_content_based_product_metadata_string_df
